...
AssistHudLayout assistHudLayout(const RegionManager& regions, double scale) {
    AssistHudLayout layout;
    scale = std::clamp(scale, 0.8, 1.4);
    if (const auto minimap = regions.getRealRegion("minimap_region")) {
        layout.box_h = 25.0 * scale;
        layout.spacing = 10.0 * scale;
        const double panel_w = std::floor(minimap->width * 0.82 * scale);
        ...
        const double original_box_h = 25.0;
        const double original_spacing = 10.0;
        const double original_large_y = std::max(0.0, minimap->top - original_box_h - original_spacing - original_box_h - 30.0);
        const double anchor_bottom = original_large_y + original_box_h + original_spacing + original_box_h;
        layout.large_y = std::max(0.0, anchor_bottom - layout.box_h - layout.spacing - layout.box_h);
        layout.mortar_y = layout.large_y + layout.box_h + layout.spacing;
    } else {
        layout.box_w = 90.0 * scale;
        layout.box_h = 25.0 * scale;
        layout.spacing = 15.0 * scale;
        layout.total_w = 4.0 * layout.box_w + 3.0 * layout.spacing;
        const double anchor_right = regions.screenWidth() - 25.0;
        const double anchor_bottom = regions.screenHeight() * 0.465 + 25.0 + 15.0 + 25.0;
        layout.x = std::max(0.0, anchor_right - layout.total_w);
        layout.large_y = std::max(0.0, anchor_bottom - layout.box_h - layout.spacing - layout.box_h);
        layout.mortar_y = layout.large_y + layout.box_h + layout.spacing;
    }
    return layout;
...
void SpecialAssistants::drawRocket(const DistanceMap& dists, const std::unordered_map<std::string, std::string>& hex,
    ...
    for (const auto& [color, dist] : dists) {
        ...
        cmds.push_back({OverlayCommand::Type::Text, cx + 36, y - 10, 0, 0, 0, std::to_string(static_cast<int>(std::round(dist))) + "m", bgr, 1, assistTextSize(config_, 18), assistTextAlpha(config_)});
    }
}
...
void SpecialAssistants::drawVss(const DistanceMap& dists, const std::unordered_map<std::string, std::string>& hex,
    ...
    for (const auto& [color, dist] : dists) {
        ...
        cmds.push_back({OverlayCommand::Type::Text, cx + 30, y - 9, 0, 0, 0, std::to_string(static_cast<int>(std::round(dist))) + "m", bgr, 1, assistTextSize(config_, 16), assistTextAlpha(config_)});
    }
}
...
void SpecialAssistants::drawCrossbow(const DistanceMap& dists, const std::unordered_map<std::string, std::string>& hex,
    ...
    for (const auto& [color, dist] : dists) {
        ...
        const auto bgr = hexToBgr(hex.count(color) ? hex.at(color) : "#FFFFFF");
        cmds.push_back({OverlayCommand::Type::Line, cx - 28, y, cx + 28, y, 0, "", bgr, 2});
        cmds.push_back({OverlayCommand::Type::Text, cx + 36, y - 10, 0, 0, 0, std::to_string(static_cast<int>(std::round(dist))) + "m", bgr, 1, assistTextSize(config_, 16), assistTextAlpha(config_)});
    }
}
...
void SpecialAssistants::drawMortar(const DistanceMap& dists, const ElevationMap& elevs,
                                   ...
                                   std::vector<OverlayCommand>& cmds) {
    const auto layout = assistHudLayout(regions_, displayValue(config_, "distance_panel_scale", 1.0));
...
void SpecialAssistants::drawDistancePanelRow(const DistanceMap& dists, const ElevationMap& elevs,
    ...
    for (const auto& color : color_order) {
        ...
        if (dist > 0.0 && dist < 121.0) {
            ...
            fill_alpha = 128;
            font_size = assistTextSize(config_, 14);
        ...
        cmds.push_back({OverlayCommand::Type::TextCenter, bx, by, bx + layout.box_w, by + layout.box_h,
                        0, text, hexToBgr("#FFFFFF"), 1, font_size, assistTextAlpha(config_)});
        ++i;
    }
}
